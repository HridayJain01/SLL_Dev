...
import { ArrowUpRight, Bell, BookOpen, Heart, Package, Sparkles, Star, Wand2 } from 'lucide-react';
 ...
 *
 * Anything that leaves the dashboard for the storefront (the catalogue) is
 * marked with an arrow, so the one jump that does exist is signposted rather
 * than sprung on the member.
 */
  ...
  return (
    <AccountPage>
      ...
      <Card>
        ...
        {isLoading ? (
          ...
          <p className="pt-[16px] font-body text-[14px] leading-[20px] text-slate-muted">
            ...
            <Link to="/account/box" className="font-semibold text-lagoon-deep hover:underline">
              Build your next box
            </Link>{' '}
            to get started.
          </p>
      ...
      {(recommended?.books.length ?? 0) > 0 && (
        <Card>
          <div className="flex items-center justify-between gap-4">
            ...
            <Link
              to="/library"
              className="inline-flex shrink-0 items-center gap-[4px] font-body text-[14px] font-semibold leading-[20px] text-lagoon-deep hover:underline"
            >
              Browse all
      ...
      <Card>
        ...
        <div className="grid gap-[12px] pt-[18px] sm:grid-cols-2 lg:grid-cols-4">
          {[
            { to: '/account/box', label: 'Build my box', icon: Package },
            { to: '/account/wishlist', label: 'My wishlist', icon: Heart },
            { to: '/account/membership', label: 'Plan & quota', icon: Star },
          ].map(({ to, label, icon: Icon }) => (
            <Link
              key={to}
              to={to}
              className="flex items-center gap-[10px] rounded-[14px] border border-[#e5e7eb] bg-white px-[16px] py-[14px] font-body text-[14px] font-medium leading-[20px] text-[#374151] transition-colors hover:border-lagoon-deep hover:text-lagoon-deep"
            >
          ...
          {/* The catalogue is the storefront, so this one does leave the
              dashboard — the arrow says so before it is clicked. */}
          <Link
            to="/library"
            className="flex items-center gap-[10px] rounded-[14px] border border-dashed border-[#d1d5db] bg-white px-[16px] py-[14px] font-body text-[14px] font-medium leading-[20px] text-[#374151] transition-colors hover:border-lagoon-deep hover:text-lagoon-deep"
          >
            <BookOpen className="h-[16px] w-[16px] shrink-0" strokeWidth={1.4} />
            <span className="flex-1">Browse the library</span>
            <ArrowUpRight className="h-[14px] w-[14px] shrink-0 text-[#9ca3af]" strokeWidth={2} />
          </Link>
        </div>
      </Card>
    </AccountPage>
